    ...
    <li style={{ display: 'flex', alignItems: 'center', gap: '.75rem', padding: '.3rem 0', borderBottom: '1px solid var(--glass-border)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {stream.trackTitle || stream.trackId} <span style={{ color: 'var(--color-text-muted)', fontWeight: 400 }}>{stream.artistName || ''}</span>
        </div>
        <div style={{ fontSize: '.9rem', color: 'var(--color-text-muted)' }}>{stream.platform} • {stream.streamedAt ? new Date(stream.streamedAt).toLocaleString() : ''}</div>
      </div>
    </li>
  )
}
