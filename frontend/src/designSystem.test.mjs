...
test('landing page uses private-ledger structure instead of SaaS motion theater', () => {
  ...
  assert.match(landingSource, /LANDING_METRIC_DEFAULTS/)
...
test('native shell keeps app switching compact and section tabs thumb reachable', () => {
  ...
  assert.match(globalCss, /body\.native-mobile-app \.sidebar \.nav-link\s*{[^}]*min-height:\s*2\.75rem/s)
  assert.match(globalCss, /body\.native-mobile-app \.sidebar \.nav-link\.active\s*{[^}]*border-top-color:\s*currentColor/s)
...
test('product copy avoids generic dashboard storytelling language', () => {
  ...
  assert.match(homeSource, /What deserves attention/)
  assert.match(homeSource, /Signals that need review/)
})
